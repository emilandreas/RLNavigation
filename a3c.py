...
sys.path.append('./gym-extensions-multitask')
import FalconLander
...
class ACNet(object):
    ...
    def _build_net(self, scope):
        ...
        with tf.variable_scope('actor'):
            l_a = tf.layers.dropout(self.s, rate=0.2)
            l_a = tf.layers.dense(l_a, 800, tf.nn.relu6, kernel_initializer=w_init, name='la')
            l_a = tf.layers.dropout(l_a, rate=0.5)
            l_a = tf.layers.dense(l_a, 800, tf.nn.relu6, kernel_initializer=w_init, name='la2')
...
class Worker(object):
    ...
    def work(self):
        global GLOBAL_RUNNING_R, GLOBAL_EP, RENDER
        total_step = 1
